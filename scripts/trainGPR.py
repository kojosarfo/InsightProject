...
for i in range(2):
    ...
    X = trafficData[['Hour','DayOfWeek','isHoliday','lat','lng']]; #Feature selection showed these features to be most predictive of the traffic volumes. Perhaps no weather parameter significantly influences pedestrian/cyclist traffic because the weather doesn't vary much between May and August.
    ...
    #Training with Gaussian process regression. The kernel used is a radial basis function (squared exponential) kernel whose length_scale is initialised to a vector of all ones: [1,1,1,1,1], the size of which is equal to the number of features (5);
    ...
    # Validation
    yPred, yStd = gprMdl.predict(X_test, return_std=True) #Gaussian process regression returns the standard devition of the prediction
    yPred = yPred.astype('int'); yPred[yPred<0]=0 #Since we are forecasting pedestrian traffic, we round of all floats and trim negative values to zero.
    
    yPred = pd.DataFrame(yPred, columns = ['forecast']); 
    yPred.index = y_test.index; yPred = yPred['forecast'];
    
    # +/- 3 times the standard deviation is used, representing a 99.7% confidence bound
